...
import { getComponent, componentBasePath } from '@/config/async_import_components_config'
...
const MenuManager = {
  getView(opts) {
    ...
    return getComponent(`${componentBasePath}menu/view/${opts.type}/${opts.cate}_${opts.name}_view.jsx`, opts.name)
  },
  ...
  getAttr() {
    ...
    return getComponent(componentBasePath + 'menu/attr/menu_attribute.js')
  },

  getControler() {
    return getComponent(componentBasePath + 'menu/controler/menu_controler.js', 'MenuControler')
  ...
  getStyle(cate, type) {
    ...
    return getComponent(`${componentBasePath}menu/style/${type}/${cate}_css.json`)
  },

  getData(type) {
    // return __webpack_require__("./components/menu/data lazy recursive ^\\.\\/.*_data\\.json$")(`./${type}_data.json`).then(module => module.default);
    return getComponent(`${componentBasePath}menu/${type}_data.json`)
  }
