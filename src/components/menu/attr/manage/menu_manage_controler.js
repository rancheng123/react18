...
// 导入模块
import ManageControler from "@/components/page/attr/manage/manage_controler";
 ...
 */
export default class MenuManageControler extends ManageControler.Component {
  constructor(props) {
    super(props);
    ...
    this.view = new MenuManage(this); //给view 入口方法绑定this
  ...
  init() {
    ...
    let {
      menuList,
      submenuList = {}
    } = menu_data; //如果submenuList是一个空数组，则把submenuList改变为对象。
  ...
  async addSubmenu() {
    ...
    const module = await Object(componentsManager)(componentType);
    ...
    if (data) {
      ...
      if (componentData) {
        ...
        this.proxy = await new AttrProxy().init(componentType); //新增控件

        const id = Dispatcher.dispatch(`${pid}_addComponent`, {
          value: componentData
        });
  ...
  removeSubmenu(index) {
    ...
    const name = document_data.name; //清空指定插入位置 

    item.skin.indexOf('submenuBox') != -1 && Dispatcher.dispatch(`drag_point`, {
      value: ''
    }); //删除控件
    ...
    Object.keys(menu_data.submenuList).length > 1 ? Dispatcher.dispatch(`${pid}_remove`, {
      value: `menu_data.submenuList.${pageid}`
    }) : Dispatcher.dispatch(`${pid}_remove`, {
  ...
  */
  async showPanel() {
    const id = this.state.current.component.id,
      parent = this.props.node.current; //保证获取到最新的皮肤 
    ...
    } = Dispatcher.dispatch(`${id}_get`); //proxy不存在则实例化并初始化代理对象

    if (!this.proxy) {
      this.proxy = await new AttrProxy().init(componentType);
    } //显示次级导航
  ...
  isShow(event) {
    ...
    }); //如果显示的是次级容器样式，设置指定容器插入 

    skin.indexOf('submenuBox') != -1 && Dispatcher.dispatch(`drag_point`, {
  ...
  showSubMenuInit(id) {
    ...
    if (fullWidth) {
      const div = window.public.dom.querySelector(`#${id}`); //只有在元素存在时，才进行逻辑处理，在mo时是不解析次级容器结构，防止没有元素而报错 
  ...
  previouslyHideSubmenu() {
    ...
    if (this.previousShowId) {
      ...
      Dispatcher.dispatch(`${this.previousShowId}_set`, {
        value: {
          showSub: false
        }
      }); //清空属性
