...
import sysToolsComp from '../../system/tools/component';
import ComponentManager from '../components_manager';
...
async function getComponent(componentType, type, skin = '') {
  ...
  if (Components[keyName] == undefined) {
    const module = await ComponentManager(componentType); //判断是否获取到了工厂对象
    ...
    if (module) {
      ...
      Components[keyName] = sysToolsComp(View, Controler);
    }
  }
...
const Util = {
  ...
  async cssParser(component, themeData, type) {
    const module = await import("./css.js");
