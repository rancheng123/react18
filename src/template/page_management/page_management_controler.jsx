...
export default class PageManagementControler extends React.Component {
  ...
  static pageManagement() {
    ...
    ReactDom.render(
      React.createElement(PageManagementControler, null),
      element
    );
  }
  /**
  ...
  init() {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (current.componentType == "em-Lightboxmodal") {
        let id = current.id,
          name =
            getIframeData.data.document_data[current.components[0].id]
              .componentName || "弹出窗口";
        lightboxArr.push({
          label: name,
          pid: id,
  ...
  clickSelected(prop = {}) {
    ...
    let close = document.querySelector("#function-modal .layer-close"),
      ...
      propertyClose = document.querySelector(
        "#component-property .layer-close"
      ),
      //属性面板关闭
  ...
  singleSetShow(data, e) {
    ...
    if (_state.tab == "lightbox") {
      ...
      let top = e.pageY - 130; //之前位置太小，易被遮档，现改变在中部显示
  ...
  changeLink(prop) {
    ...
    const promise = Promise.all(
      /*! import() | link_controler */[
        __webpack_require__.e(2),
        __webpack_require__.e(3),
        __webpack_require__.e("link_controler"),
      ]
    ).then(
      __webpack_require__.bind(
        null,
        /*! ../../system/function/link/link_controler */ "./system/function/link/link_controler.js"
      )
    );
    promise.then((module) => {
      module.LinkControler &&
        module.LinkControler.link({
    ...
    resourceManager("icon").then((module) => {
      module.resource({
        selected: (datas) => {
          ...
          this.pageAjax(ajaxData);
          const close = document.querySelector(".layer-close"); //切换icon时关闭属性面板 为了解决（切换icon后会刷新页面并且更改操作按钮的状态 然后当用户再次点击属性面板内的皮肤时 这时在页面中的皮肤是无法选中的，所以需要让用户重新打开操作按钮）panel无法选择问题 
  ...
  pageSetDelete() {
    let state = this.state || {},
      ...
      text =
        window.public.lang["confirmDelete"] + label + window.public.lang["you"];
    Layer.alert({
      area: ["420px", "225px"],
      skin: "",
      close: true,
    ...
    fetch("/desktop/index.php/Edit/Pages/pagesEditOperat", {
      ...
      .then((data) => {
        ...
        if (msg && msg.list) {
          ...
          if (item) {
            let pageNameId = "",
              paramType = param.type; //类型为新增和复制页面时，显示页面重命名结构
            ...
            window.public.pageName = item.label; //给页面名称赋值 
            //restore 是页面回收站还原  recovery 是清空回收站 
  ...
  renameFailed(suc) {
    if (
      suc == "7000" &&
      this.renameId &&
      this.renameId == this.state.currentPageId
    ) {
      this.setState({
  ...
  originHandle(data) {
    ...
    data.forEach((item) => {
      if (item.pid == this.original.pid) {
        this.current = MenuDragTools.createPlac(
          this.original.pid,
          item.parent_id
        );
  ...
  rightHandle(plac, data, currentX, currentY) {
    ...
    if ((level < this.maxLevel && hasChild === -1) || this.maxLevel === -1) {
      if (prev) {
        ...
        const _data = MenuDragTools.listorder([
          ...diffList,
          ...sameList,
          this.current,
        ]);
  ...
  dragEndSameName(plac, data) {
    return data.find((item) => {
      return (
        item.parent_id == plac.parent_id &&
        item.label == this.original.label &&
        item.pid != plac.pid
      );
    });
  }
  ...
  dragEnd() {
    ...
    if (typeof plac !== "undefined") {
      ...
      const getSameList = MenuDragTools.getSameList(
        MenuDragTools.data,
        plac.parent_id
      );
  ...
  dragStart(event) {
    ...
    while (
      !(_target.tagName == "LI" && _target.getAttribute("data-element") == "1")
    ) {
    ...
    this.original = MenuDragTools.findEle(
      MenuDragTools.data,
  ...
  dragMove(event) {
    const { currentX, currentY, offsetX, offsetY } = SingleDrag.move(event);
    const { scrollTop } = this.pageMenu;
