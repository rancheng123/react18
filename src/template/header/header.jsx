...
export default class Header {
  ...
  render() {
    ...
    //   className: "iconfont"
    // }, "\uE60D")));
    console.log(this.controler.switchPcEdit, 'hahahha ')
    return (

      <header id="ediHeader">
        <div className="ediHeLeft">
          <img src={logo} width="128" style={{ paddingTop: "10px" }} />
        ...
        <div className="ediHeRight">
          <ul className="e_ulBox">
            {
              this.controler.btn_list.map((item, index) => {
                if (!item) return null
                return (
                  <li
                    key={index}
                    id={item.event}
                    title={item.title}
                    // onClick={this.controler[item.event].bind(this.controler)}
                  >
                    {
                      item.event === 'rulerShow' ? (
                        <a>
                          <svg t="1582531184791" width="20" height="20" className="icon" fill='#7E4DF7'
